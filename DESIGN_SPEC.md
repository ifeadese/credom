...
3. **Vision / Mission** — full-bleed split, `min-height:440px`, eyebrow at top, statement at bottom: **Vision** (teal) "Redefine brand impact in Africa." + copy; **Mission** (gold, brown eyebrow) "Connection that drives growth." + two paragraphs. Each panel carries a **laughter-portrait watermark** (`public/images/about/laughter-vision.jpg`, `laughter-mission.jpg` — greyscale Pexels stock, free licence): pinned to the panel's outer edge in a `min(62%, 620px)`-wide column, `object-cover`, `mix-blend-mode: luminosity` at 22% opacity so the panel colour tints it, and masked with `linear-gradient(to left, #000 45%, transparent)` so it fades out under the copy. Decorative only (`aria-hidden`, empty alt).
...
1. **Hero** — the shared `components/PageHero.tsx` (same markup as Contact): white, bottom hairline, eyebrow "Schedule a Chat"; H1 "Let's talk. / **Pick a time.**" (`clamp(76px,10.4vw,150px)`, line 0.8); intro paragraph (`lib/content.ts` `scheduleIntro`). Faint "CHAT" watermark (Rokkitt 900, gold at 9%) bottom-right.
