...
2. **Booking** — inside the hero section, in the same content column below the intro (`clamp(40px,5vw,64px)` above it; no separate section): CREDOM's Cal.com booking (`credomlimited/30min`) as an **inline embed** (`components/CalEmbed.tsx`, a client component porting Cal.com's loader; identifiers in `lib/cal.ts`), no border or radius of its own. The wrapper is capped at `1040px` — the width of Cal.com's three-column month-view card (meta 280 · calendar 480 · slots 280 on frames ≥ 1024px) — and left-aligned, so the card's left edge sits on the column's left edge with the headings instead of floating centred inside a wider frame. Month-view layout, slots view on small screens; Cal.com sizes the frame to its content. Event-type details are shown (Cal.com's `hideEventTypeDetails: false`). Brand colour `cal-brand` = gold from `lib/tokens.ts` (applied to Cal.com's dark theme per the embed config). Query params on `/schedule` are forwarded to the booking.
