...
6. **What we've done** — eyebrow, H2 "Work that moved people.", then the single AltDrive case study laid out as **image + plain text, no card**: 4:3 image (radius 4) in a `minmax(280px,2fr)` column, copy in a `3fr` column (gap 56, vertically centred) — gold-deep category eyebrow, ink H3, body `#4A443D`, gold-deep bold takeaway, gold "Read more". Then "Businesses we've served" + the logo rail (logos left-aligned when stacked on phones).
