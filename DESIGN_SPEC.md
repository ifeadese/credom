...
2. **Body** — white, no vertical padding, **no Container**: the **Calendly inline widget** (`components/ScheduleEmbed.tsx`, `react-calendly` loaded client-side only) runs edge to edge, 720px tall (900px on phones), no border or radius. Event-type details, landing-page chrome and GDPR banner all hidden so only the calendar + time picker show; the "What to expect" copy (01 A quick intro · 02 Your moment · 03 Next steps + email/phone fallback) lives in the Calendly event description and reaches invitees via Calendly's confirmation. Primary colour gold, text ink, background white. Event URL from `NEXT_PUBLIC_CALENDLY_URL` (fallback in `lib/calendly.ts`).
