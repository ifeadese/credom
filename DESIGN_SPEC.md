...
2. **Booking** — inside the hero section, in the same content column below the intro (`clamp(40px,5vw,64px)` above it; no separate section): CREDOM's Cal.com booking (`credomlimited/30min`) as an **inline embed** (`components/CalEmbed.tsx`, a client component porting Cal.com's loader; identifiers in `lib/cal.ts`), no border or radius of its own. Month-view layout, slots view on small screens; Cal.com sizes the frame to its content. Event-type details are shown (Cal.com's `hideEventTypeDetails: false`). Brand colour `cal-brand` = gold from `lib/tokens.ts` (applied to Cal.com's dark theme per the embed config). Query params on `/schedule` are forwarded to the booking.
